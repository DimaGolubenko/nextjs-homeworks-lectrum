...
export class NextScriptCustom extends NextScript {
  render() {
    ...
    const nextDataScripts = initialLoadScripts.filter(({ props }) => props && nextDataFilter(props));
    ...
    const polyfillJSX = polyfill.map(({ props }) => <script key={`initialScript${props.src}${props.id}`} {...props} src={props.src} />);
    ...
    const nextDataScriptsJSX = nextDataScripts.map(({ props }) => <script key={`initialScript${props.src}${props.id}`} {...props} src={props.src} />);
    return (
      <>
        {polyfillJSX}
        {nextDataScriptsJSX}
