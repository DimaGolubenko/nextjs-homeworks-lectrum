...
export const serverGraphqlErrorLogger = (query, err, context) => {
  ...
  const responseCode = err.networkError && "statusCode" in err.networkError && err.networkError.statusCode;

  const responseMessage = err.networkError && "result" in err.networkError && err.networkError.result.message;
  ...
  if (isDevelopment) {
    ...
    if (graphQlErrorType) {
      ...
      const graphQLValidationsErrors = err.graphQLErrors && err.graphQLErrors.length > 0 && JSON.stringify(err.graphQLErrors);
